function [P_total] = faceOrientation(r_satsun)
 %faceOrientation - Calculates 12U power generation based on a given sun vector.
...
%Initialize Normal Vectors to Each Face
N1 = [1,0,0];
N2 = [0,1,0];
N3 = [0,0,1];
N4=-N1;
...
%Dot products of sun vector and normal vectors
dot1 = dot(r_satsun,N1);
dot2 = dot(r_satsun,N2);
dot3 = dot(r_satsun,N3);
dot4 = dot(r_satsun,N4);
dot5 = dot(r_satsun,N5);
dot6 = dot(r_satsun,N6);

%magnitudes

mag1 = norm(r_satsun)*norm(N1);
mag2 = norm(r_satsun)*norm(N2);
mag3 = norm(r_satsun)*norm(N3);
mag4 = norm(r_satsun)*norm(N4);
mag5 = norm(r_satsun)*norm(N5);
mag6 = norm(r_satsun)*norm(N6);
...
%Initialize if each face is exposed to the sun
F1 = 0;
F2 = 0;
F3 = 0;
F4 = 0;
...
%Maximum power produced by one of the cells 
P_pan = 474.324; %W/m^2

P1 = P_pan.*F1.*A1.*costh1;
P2 = P_pan.*F2.*A2.*costh2;
P3 = P_pan.*F3.*A3.*costh3;
P4 = P_pan.*F4.*A4.*costh4;
P5 = P_pan.*F5.*A5.*costh5;
P6 = P_pan.*F6.*A6.*costh6;
